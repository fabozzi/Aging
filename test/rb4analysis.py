...
# dictionary with list of RB4 ID for each Wheel
    ...
    if not( (chambID[1]) in badChambers ) :
        if (chambID[1]).startswith("W"):
            ...
            if ((chambID[1]).find("RB4")) > -1 :
                rb4RollsDict[ chambID[0] ] = chambID[1]
                if (chambID[1]).startswith("W-2"):
                    (rb4Rolls_InW[ "W-2" ]).append( chambID[0] )
                elif (chambID[1]).startswith("W-1"):
                    (rb4Rolls_InW[ "W-1" ]).append( chambID[0] )
                elif (chambID[1]).startswith("W+0"):
                    (rb4Rolls_InW[ "W+0" ]).append( chambID[0] )
                elif (chambID[1]).startswith("W+1"):
                    (rb4Rolls_InW[ "W+1" ]).append( chambID[0] )
                else:
                    (rb4Rolls_InW[ "W+2" ]).append( chambID[0] )
...
rb4ids = rb4RollsDict.keys()
re1Ring3ids = re1Ring3RollsDict.keys()
barrelids = barrelRollsDict.keys()
endcapids = endcapRollsDict.keys()

# NOW WE HAVE THE LISTS OF BARREL / ENDCAP / RB4_ROLLS (reference) / RE1Ring3_ROLLS (reference)
...
for i in xrange(fittree.GetEntries()):
    fittree.GetEntry(i)
#    if (rpcfit.chi2/float(rpcfit.ndof) < 10) and (rpcfit.chi2/float(rpcfit.ndof) > 2) :
    if (rpcfit.chi2/float(rpcfit.ndof) < 2) and (rpcfit.refp > 0.9) and (math.fabs(rpcfit.p1) < 0.02) :
        print rpcfit.rollidtree, rpcfit.p1, " +/- ", rpcfit.p1err, rpcfit.refp, " +/- ", rpcfit.refperr
        nroll_sel = nroll_sel + 1
        roll_sel.append( (rb4RollsDict[ str( rpcfit.rollidtree ) ] ).replace('+','p').replace('-','m')  )
print nroll_sel

outplotdir = "rb4norm_inbox_chirlt2"
...
for rollname in roll_sel :
    ...
    rfile_in.GetObject("eff_"+rollname+"_norm",myh)
    myh.Draw("AP")
